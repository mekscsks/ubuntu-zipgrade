...
- This section is intentionally _outside_ the bubble grid bounding box used by `_detect_bubbles()`, so OCR/handwriting never interferes with bubble detection.
...
2. If restoring into the same project, first export the _current_ (corrupted) state as a safety snapshot.
